...
$PAGE->navbar->add(get_string('channels', 'block_edupublisher'), new moodle_url('/blocks/edupublisher/pages/list.old.php', array()));
if (!empty($channel)) {
...
if (empty($channel)) {
    ...
    */
    ...
    for ($a = 0; $a < $pages; $a++) {
        $url = new \moodle_url('/blocks/edupublisher/pages/list.old.php', array('channel' => $channel, 'page' => $a, 'search' => $search));
        $pagination->pages[$a] = [
            'active' => ($page == $a) ? '1' : '0',
            'label' => ($a + 1),
            'link' => $url->__toString(),
